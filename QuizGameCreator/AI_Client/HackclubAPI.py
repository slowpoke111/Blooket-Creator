...
class HackclubAPI(AIClient):
    ...
    def __init__(self) -> None:
        ...
        self.client: OpenAI = OpenAI(
            api_key=api_key, base_url="https://ai.hackclub.com/proxy/v1"
        )

    def _prompt_api(self, question: str, correct_answer: str) -> str:
    ...
    def generate_distractors(
        ...
        try:
            content = self._prompt_api(question, correct_answer)
            ...
            distractors: List[Optional[str]] = [
                line
                for line in lines
                if line and not line.lower().startswith("distractor")
            ]
            ...
            if status_code == 429:
                print(
                    f"Rate limit exceeded (429): Too many requests. Please wait before making more requests."
                )
            else:
